...
class UserActiveCases extends ConsumerStatefulWidget {
  final String aadharOrPhone;

  const UserActiveCases({
    Key? key,
    required this.aadharOrPhone,
  }) : super(key: key);

  @override
  _UserActiveCasesState createState() => _UserActiveCasesState();
}

class _UserActiveCasesState extends ConsumerState<UserActiveCases> {
  late Future<List<CaseModel>> _casesFuture; // Declare future variable

  @override
  void initState() {
    super.initState();
    _casesFuture = CloudFirestoreClass().fetchUserCasesFromDatabase(
